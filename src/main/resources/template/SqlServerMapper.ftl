...
<mapper namespace="${content.dao.classPackage}.${content.dao.className}">
	...
	<select id="${content.dao.item.select.value}" parameterType="${content.assist.classPackage}.Assist" resultMap="${content.mapper.item.resultMap.value}">
	  select * from
      (
        select <if test="distinct !=null">${r'${distinct}'}</if> ROW_NUMBER() over(<choose><when test="order!=null">${r'${order}'}</when><otherwise>order by ${content.entity.primaryKeyAttr.columnName}</otherwise></choose>) as page,
          <choose>
