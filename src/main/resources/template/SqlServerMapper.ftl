 ...
 * @param <T>
 */
public abstract class AbstractSQLSqlServer<T> {

	private final Logger LOG = LogManager.getLogger(AbstractSQLSqlServer.class);
	 ...
	 * @return 返回:sql or sql与params
	 */
	public SqlAndParams selectAll(SqlAssist assist) {// 如果Assist为空返回默认默认查询语句,反则根据Assist生成语句sql语句
		if (assist == null) {
			...
			if ((assist.getRowSize() == null && assist.getStartRow() == null)) {
				...
				String sql = MessageFormat.format("select {0} {1} from {2} {3}", distinct, column, tableName(), condAndOrder);
				...
				String sql = MessageFormat.format("select * from ( select {0} ROW_NUMBER () over ({1}) AS page , {2} from {3} {4} ) result {5}",
	...
	public SqlAndParams selectByObj(T obj, String resultColumns, boolean single) {
		...
		StringBuilder sql = new StringBuilder(
				MessageFormat.format("{0} {1} from {2} ", prefix, resultColumns == null ? columns() : resultColumns, tableName()));
		JsonArray params = null;
