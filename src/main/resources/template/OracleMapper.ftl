 ...
 * @param <T>
 */
public abstract class AbstractSQLOracle<T> {

	private final Logger LOG = LogManager.getLogger(AbstractSQLOracle.class);
		...
		if (assist == null) {
			...
			if (assist.getRowSize() == null && assist.getStartRow() == null) {
				...
				String sql = MessageFormat.format(
						"select * from ( select  rownum as page,result.* from (  select {0} {1} from {2} {3} {4} ) result {5} ) {6} ", distinct, column,
						tableName(), cond, order, limit, limit2);
